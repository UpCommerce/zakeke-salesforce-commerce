 ...
 */
function getTotalQuantity(items, isCartPage) {
    var showPatchProduct = Site.getCurrent().getCustomPreferenceValue('ZAKEKE_ShowPatchProduct') || false;
    ...
    collections.forEach(items, function (lineItem) {
        ...
        if(lineItem.getCustom().ZAKEKE_IsPatchLineItem){
            if(!empty(isCartPage) && isCartPage == true){
                return;
            }
            if(showPatchProduct == false){
                return;
            }
        }
 ...
 * @param {string} view - the view of the line item (basket or order)
 */
function ProductLineItems(productLineItems, view, isCartPage) {
    if (productLineItems) {
        this.items = createProductLineItemsObject(productLineItems, view);
        this.totalQuantity = getTotalQuantity(productLineItems, isCartPage);
    } else {
        this.items = [];
